...
# Сколько ждать webhook от Legnext перед тем как считать запрос потерянным (секунды).
# 120с — запас на медленный v8.1 fast (норма ~80с), плюс время на доставку webhook
# и обработку картинок в api_server. Меньшее значение даёт race с retry watchdog'а.
MJ_WATCHDOG_TIMEOUT = 120

# Таймаут для тихого retry на v7+turbo. У v8.1 нет turbo, поэтому первый таймаут на v8.1 fast,
# а второй — на v7+turbo. Итого макс. ожидание до финального сообщения = 120 + 90 = 210с (3:30).
